    ...
    class TileEditor : EditorWindow
    {
        ...
        private void DrawWindow()
        {
            ...
            if (mapEditor)
            {
                ...
                {
                    var selectedTile = mapEditor.GetSelectedTile();
                    mapEditor.targetMap.onTileModified.Invoke(mapEditor.GetSelectedTile());
                    if (selectedTile.data.gameObject != null)
                    {
                        selectedTile.data.gameObjectName = selectedTile.data.gameObject.name;
                    }

                    if (mapEditor.AutosaveEnabled())
                    {
                        JsonMapTool.SaveMap(mapEditor.targetMap.data);
                    }
                }
            }
        }
